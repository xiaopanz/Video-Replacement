...
int main(int argc, char **argv) {
  ...
  // std::vector<std::pair<size_t, size_t>> ads_intervals = {{0, 449},
  //                                                         {6000, 6449}};
  ...
  const std::string nfl_logo_path =
      "data/dataset-002/dataset2/Brand Images/nfl_logo.rgb";
  const std::string nfl_logo_mask = "./nfl_mask.bmp";

  const std::string mcdonald_logo_path =
      "data/dataset-002/dataset2/Brand Images/Mcdonalds_logo.raw";
  const std::string mcdonald_logo_mask = "./mcdonalds_mask.bmp";

  const std::string nfl_ads_path =
      "data/dataset-002/dataset2/Ads/nfl_Ad_15s.rgb";
  const std::string mcdonald_ads_path =
      "data/dataset-002/dataset2/Ads/mcd_Ad_15s.rgb";

  // Read Logo
  std::vector<cv::Mat> nfl_logo_frames = ReadRGBFrames(nfl_logo_path);
  std::vector<cv::Mat> mcdonald_logo_frames = ReadRGBFrames(mcdonald_logo_path);
  std::vector<cv::Mat> nfl_ads_frames = ReadRGBFrames(nfl_ads_path);
  std::vector<cv::Mat> mcdonald_ads_frames = ReadRGBFrames(mcdonald_ads_path);

  auto nfl_logo_boxes = detect_nfl::FindLogo(
      rgb_frames_no_ads, nfl_logo_frames.at(0), nfl_logo_mask);
  auto mcdonald_logo_boxes = detect_mcdonald::FindLogo(
      rgb_frames_no_ads, mcdonald_logo_frames.at(0), mcdonald_logo_mask);

  AddLogoRect(rgb_frames_no_ads, mcdonald_logo_boxes, "mcd");
  AddLogoRect(rgb_frames_no_ads, nfl_logo_boxes, "nfl");

  std::vector<std::vector<cv::Mat>> all_ads(
      {mcdonald_ads_frames, nfl_ads_frames});
