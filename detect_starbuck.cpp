...
#pragma omp parallel for collapse(2)
    for (int i = 0; i < frame.rows - curr_height; i += 1) {
      for (int j = 0; j < frame.cols - curr_width; j += 1) {
        ...
        cv::MatND sub_img_hist = GetColorHistogram(sub_img_hsv, scaled_mask);
...
MeanshiftTrackAcrossFrame(const std::vector<cv::Mat> &rgb_frames,
  ...
  tracking_window.x = rect.x + rect.width / 2;
  tracking_window.width = rect.width / 2;
  tracking_window.y = rect.y + rect.height / 3;
  ...
  result[frame_id] = rect;

  // cv::Mat frame_clone = rgb_frames[frame_id].clone();
