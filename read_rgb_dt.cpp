#include "WavReader.h"
#include "detect_starbuck.h"
#include "detect_subway.h"
...
FindAdsShots(const std::vector<size_t> &shots_start,
  ...
  std::vector<bool> original_ads_candiate = ads_candidates;
  for (size_t i = 0; i < shots_start.size(); i++) {
    size_t shot_length;
    if (i == shots_start.size() - 1) {
      shot_length = rgb_frames.size() - shots_start[i];
    ...
    if (!ads_candidates[i] && shot_length < 240) {
      ...
      // likely an ad
      if (i > 0 && i < shots_start.size() - 1 && original_ads_candiate[i - 1] &&
          original_ads_candiate[i + 1]) {
        ads_candidates[i] = true;
      } else if (i < shots_start.size() - 1 && original_ads_candiate[i + 1]) {
      ...
      } else if (i > 0 && original_ads_candiate[i - 1]) {
        ads_candidates[i] = true;

        // double curr_saturation =
        // GetAvgSaturation(rgb_frames[shots_start[i]]); double prev_saturation
        // =
        //     GetAvgSaturation(rgb_frames[shots_start[i - 1]]);
        // // If it's a middle shot, we can compare the prev and next
        // bool cond_1 =
        //     (i > 0) &&
        //     ((std::abs(curr_saturation - prev_saturation) <
        //       std::abs(curr_saturation -
        //                GetAvgSaturation(rgb_frames[shots_start[i - 1]]))));
        // // If it's a start shot, we can only compare the curr and next by a
        // // threshold
        // bool cond_2 =
        //     (i == 0) && (std::abs(curr_saturation - prev_saturation) < 20);
...
void CreateNFLMask() {
  cv::Mat mask(cv::Size(width, height), CV_8UC1, cv::Scalar(0));
  for (int i = 0; i < height; i++) {
    for (int j = 0; j < width; j++) {
      if (j > width / 4 && j < width / 4 * 3) {
...
int main(int argc, char **argv) {
  ...
  std::vector<FrameInfo> frame_info_vec;
  std::vector<size_t> shots_start = DivideShotsOMP(rgb_frames, frame_info_vec);
  auto ads_intervals = FindAdsShots(shots_start, rgb_frames, frame_info_vec);

  // std::vector<std::pair<size_t, size_t>> ads_intervals = {
  //     {3220, 3668}, {5727, 6177}, {7322, 7798}};
  // 448, 450, 476
  ...
  const std::string starbuck_logo_path =
      "data/dataset-test/test1/Brand Images/starbucks_logo.rgb";
  const std::string subway_logo_path =
      "data/dataset-test/test1/Brand Images/subway_logo.rgb";

  const std::string starbuck_ads_path =
      "data/dataset-test/test1/Ads/Starbucks_Ad_15s.rgb";
  const std::string subway_ads_path =
      "data/dataset-test/test1/Ads/Subway_Ad_15s.rgb";

  // // Read Logo
  std::vector<cv::Mat> starbuck_logo_frames = ReadRGBFrames(starbuck_logo_path);
  std::vector<cv::Mat> subway_logo_frames = ReadRGBFrames(subway_logo_path);
  std::vector<cv::Mat> starbuck_ads_frames = ReadRGBFrames(starbuck_ads_path);
  std::vector<cv::Mat> subway_ads_frames = ReadRGBFrames(subway_ads_path);

  auto starbuck_logo_boxes = detect_starbuck::FindLogo(
      rgb_frames_no_ads, starbuck_logo_frames.at(0), "");
  auto subway_logo_boxes = detect_subway::FindLogo(
      rgb_frames, subway_logo_frames.at(0), "subway_mask.png");

  AddLogoRect(rgb_frames_no_ads, starbuck_logo_boxes, "starbuck");
  AddLogoRect(rgb_frames_no_ads, subway_logo_boxes, "subway");

  std::vector<std::vector<cv::Mat>> all_ads(
      {starbuck_ads_frames, {}, subway_ads_frames});
