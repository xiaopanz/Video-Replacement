...
std::vector<cv::Mat> FilterRGBFrames(
...
int main(int argc, char **argv) {
  ...
  // std::vector<FrameInfo> frame_info_vec;
  // std::vector<size_t> shots_start = DivideShotsOMP(rgb_frames,
  // frame_info_vec); auto ads_intervals = FindAdsShots(shots_start, rgb_frames,
  // frame_info_vec); std::vector<cv::Mat> rgb_frams_no_ads =
  //     FilterRGBFrames(rgb_frames, ads_intervals);
  // WriteRGBFrames(rgb_frams_no_ads, argv[3]);
  // std::vector<cv::Mat> test_frames = {rgb_frames[2700]};
  // std::vector<cv::Mat> test_frames = {rgb_frames[4350]};

  // Read Logo
  std::vector<cv::Mat> logo_frames = ReadRGBFrames(argv[5]);

  // FindLogo2(rgb_frames, logo_frames.at(0), argv[6]);
  // FindLogoSingleFrame(rgb_frames[2220], logo_frames.at(0), argv[6]);
  // FindLogoSingleFrame(rgb_frames[2670], logo_frames.at(0), argv[6]);

  // detect_mcdonald::FindLogoSingleFrame(rgb_frames[2000], logo_frames.at(0),
  //                                      argv[6]);

  // detect_mcdonald::FindLogo(rgb_frames, logo_frames.at(0), argv[6]);

  // detect_nfl::FindLogo({rgb_frames[2220]}, logo_frames.at(0), argv[6]);
  // detect_subway::FindLogo(rgb_frames, logo_frames.at(0), "");

  detect_starbuck::FindLogo({rgb_frames[5340]}, logo_frames.at(0), "");

  // cv::Mat mask = cv::Mat(logo_frames.at(0).size(), CV_8UC1, cv::Scalar(1));
  // mask = cv::imread(argv[6], cv::IMREAD_GRAYSCALE);

  // DetectMcDonald::DetectMcDonald(rgb_frames, logo_frames.at(0), mask);

  // if (argc > 5) {
  //   if (argc == 7) {
  //     FindLogo(rgb_frames, argv[5], argv[6]);
  //   } else {

  //     FindLogo(rgb_frames, argv[5], "");
  //   }
  // }
  // OutputFrames(rgb_frames);
  // cv::imwrite("logo_hist.png", PlotHistogram(logo_hist));

  // HistogramObjectDetection(logo_hsv, logo_hist, rgb_frames[2700]);
  // HistogramObjectDetection(logo_hsv, logo_hist, rgb_frames[4351]);
  // for (int i = 0; i < rgb_frames.size(); i += 150) {

  //   double score = HistogramObjectDetection(logo_hsv, logo_hist,
  //   rgb_frames[i]);
  //   // if (i % 300 == 0) {
  //   // std::cout << "Progress: " << i << std::endl;
  //   // }
  //   // if (score > 1.2) {
  //   std::cout << "Frame: " << i << " score=" << score << std::endl;
  //   // }
  // }

  // DivideShotsOMP(rgb_frames);
  // OutputFrames(rgb_frames);
