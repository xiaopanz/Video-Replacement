...
FindLogo(const std::vector<cv::Mat> &rgb_frames, cv::Mat logo,
  ...
  std::unordered_map<size_t, cv::Rect> logo_boxes;
  for (size_t i = 0; i < rgb_frames.size(); i += 60) {
    auto [score, rect] = HistogramObjectDetection(logo, logo_hsv, logo_hist,
                                                  rgb_frames[i], mask);
    std::cout << "Frame: " << i << " " << std::setprecision(10) << score
              << std::endl;
    if (score > 0.092) {
      logo_boxes = MeanshiftTrackAcrossFrame(rgb_frames, i, rect);
